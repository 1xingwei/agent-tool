...
日期：2026-09-15
状态：**已落地**（三项目标全部实现；回填日期 2026-09-17）
项目：agent-service-toolkit（面试展示项目）

> 落地回填：①`scripts/create_chroma_db.py` 已重做（多格式 + `CHROMA_DIR` + 破坏性删除改 opt-in），
> `src/agents/tools.py` 的加载器已可配置；②`src/memory/sqlite.py` 已换成官方 `AsyncSqliteStore`，
> 持久化实测成立；③两个 supervisor 的 `web_search` 已换成真实 DuckDuckGo 搜索。
> 配套测试 `tests/test_ingest.py`、`tests/test_sqlite_store.py` 已就位。
> 后续在同一个 supervisor 上又修掉了一个更硬的缺陷（转交必 500），见
> `docs/11-Agent能力完善方案.md` 与 `docs/10-审核与修复总账.md`。
...
> 与后续方案的关系：`docs/15-记忆与检索层-对标核实与行动清单.md` §7.3 的 P1-1
> 提出把向量库抽象成 `VectorStore`（默认 Chroma、有 Postgres 则切 pgvector）。
> 两者不冲突——本文的 YAGNI 判的是「为面试叙事加分」，而那是为「摆脱 Chroma
> 原型定位、支持混合检索」而做，属于另一条理由。真要动手时以文档 15 的排期为准。
